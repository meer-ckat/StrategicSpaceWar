...
public static partial class Ballistics
{
    ...
    /// <summary>
    /// 밀린 파편이 예산의 이 배를 넘으면 그 틱은 예산을 무시하고 따라잡는다.
    ///
    /// **없으면 갈리는 동안 빚이 영원히 자란다** - 생산이 예산을 계속 넘으면 큐가 끝없이
    /// 길어지고, 피해가 몇 초씩 밀려 들어와 "맞았는데 한참 뒤에 부서지는" 배가 된다.
    /// 스파이크를 눕히는 것과 빚을 지는 것은 다른 일이고, 이 숫자가 그 선이다.
    /// </summary>
    public const int SpallBacklogCatchUp = 4;
