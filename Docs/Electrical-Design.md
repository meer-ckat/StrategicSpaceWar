...
전기설비 과목의 자리. 전선 하나에 차단기 하나, 정격 = 전선 허용전류(`BreakerAmps` 100 A). 새 증상: **손상 없이** 포가
죽는다(과부하 트립). 새 결정: 정지 회로도에서 트립된 전선을 골라 둔 채 한 번 더 클릭 = 올린다. 원인이 그대로면 다시 내려간다.

| 어디 | 무엇 |
|---|---|
| `Ship.TripBreakers` | 풀이마다 전선의 최대 구간 전류 / 정격. 1 초과면 `((I/Ir)² − 1)·dt` 적분(열동), `BreakerInstantMul`(10×) 이상이면 즉시(전자). 적분이 `BreakerTripSeconds × 3`에 닿으면 `tripped`. 정격 밑에선 초당 `BreakerCoolPerSecond`씩 빠진다 |
| `SegmentIntact` | `run.tripped`면 그 전선 전부 alive false. `CutCause.Tripped` |
| `Ship.ResetBreaker(wire)` | 사람이 올린다. `Inspect.Click`이 같은 전선 재클릭에서 부른다 |
| UI | WireView 트립 = HEAT(끊김 BREACH와 다른 색). INSPECT 전선: 상태 줄 + 차단기 줄(정격·누적 %). 피탄 판독 `BREAKER TRIP`, RunLog `BreakerTrip` |
| 왜 100 A | m12 여덟 문 동시 기동(6.3 A × 8 = 50 A)은 통과해야 하고, 단락(6.4 kA)은 전선이 타기(2 kA, §4) 전에 내려가야 한다 |

전선 열(§4)은 그대로 있다 - 차단기가 없는 자리(정격 위 2 kA까지)는 여전히 탄다. 차단기가 먼저 내려가면 타지 않는다.

### 9.1 단락은 어디서 오나 (2026-09-19)

전투에서 단락을 본 적이 없던 이유: 탄이 전선 서브셀을 죽이면 **끊김(open)**만 됐다. 실제 선체 케이블은 눌리면
선체에 붙는다. 그래서 **판 위에서 관통당한 구간(`CutCause.Breached`)은 단락**이다 - 재빌드 때 구간 가운데에 접지
사고 노드(Load, `WireFaultOhms` 1 mΩ)를 놓고 양쪽을 반씩 잇는다(`efault` 간선). 탄 것(`Burned`)·판 소실·잔해 이탈·
구멍은 붙을 선체가 없어 열린 회로 그대로다. 단락 구간 수가 바뀌면 재빌드(노드는 재빌드에서만 생긴다).

결과가 규칙에서 나온다: 원자로 → 사고점 경로가 4.5 kA를 흘려 그 구간들이 약 4 s 뒤 같이 타고(§4, 판 서브셀도
`Burn`), 원자로 내부저항 0.05 Ω에 6.4 kA면 단자 전압이 바닥이라 **그 몇 초는 배 전체가 정전**이다. 차단기(§9)가 있으면
100 ms 안에 그 전선만 내려가고 나머지는 산다 - 리셋하면 다시 내려간다(사고가 그대로다). 정비가 판을 고쳐야 풀린다.

## 10. M5 - 병렬 운전과 버스 타이 (2026-09-20)

전기이론의 마지막 자리. 규칙 셋이 더 들어가고 "전력 인카운터"는 그 셋에서 나온다 - 전용 이벤트가 없다.

| # | 규칙 | 어디 |
|---|---|---|
| 1 | 섬의 뿌리가 아닌 전원은 **역기전력 부하**다. 모터와 같은 (rInt, emf) 쌍으로 `PowerNet.Solve`에 간다 | `PowerGraph.Solve` 두 줄 |
| 2 | 원자로 기전력은 체력을 따라 처진다. `PhaseVoltage × lerp(ReactorDroopFloor, 1, Health01)` | `Ship.RebuildPowerNet` |
| 3 | 역전류(`i > 0`인 비뿌리 전원)가 내부저항에서 내는 열이 손상이다 | `Ship.ReverseFeed` |

**부호 하나가 두 상황을 가른다.** 전압이 같으면 케이블 강하만큼 emf가 높은 쪽이 이겨 `i < 0`(같이 문다, 부하 분담).
한쪽이 상해서 처지면 `i > 0`이 되어 **먹힌다** - 그 열이 더 상하게 하고, 더 상하면 더 처지는 가속 고리라 유폭까지 간다.
분기문도 상태 기계도 없고, 그래서 원자로 둘을 한 버스에 다는 설계에 값이 생긴다.

**버스 타이 = 차단기를 손으로 여는 것.** M4의 재클릭을 토글로 넓혔다(`Ship.OpenBreaker`): 내려간 것은 올리고 살아 있는
것은 내린다. 죽어가는 원자로를 버스에서 떼거나 단락 구간을 격리하는 것이 같은 동작 하나다. 스스로 내려간 것(HEAT)과
내가 연 것(TELEMETRY)은 색이 다르고, 사고가 남아 있으면 `사고` 줄이 따로 뜬다 - 올리면 무엇이 다시 일어나는지 알고 누르게.

**파지직**은 인카운터가 아니라 판독이다. `WireArcAmps`(200 A)를 넘는 구간이 8틱에 한 번꼴로 `BlastFlashSmall`을 띄운다
(시드는 틱·간선 번호뿐이라 재현된다). 소리는 아직 없다 - 클립이 Blow·Cannon·Communication·Critical·Penetrate뿐이라
아크에 맞는 것이 없다.

남은 것: 릴레이·접촉기 Device(실기 시퀀스), 비상전원, 노후 배선을 들고 태어나는 구조선(코드 0 - 손상 저장본이 곧 데이터).

### 10.1 배선판 (2026-09-20, 오너 설계)

정지 회로도에서 고른 것에 손잡이가 딸린다. **INSPECT(아래 가운데)는 값을 읽는 자리, 배선판(왼쪽)은 손잡이다** -
한 패널에 섞으면 "지금 꺼짐"인지 "끄기"인지가 안 갈린다. 그래서 스위치의 램프가 현재 상태고 글자는 누르면 일어날 일이다.

| 고른 것 | 계기 | 스위치 |
|---|---|---|
| 전선 | 전류 · 정격 · 누적 I²t · 온도 | 차단기 개방/투입 (버스 타이) |
| 원자로 | 계통(모선 급전/병렬/정지/분리됨) · 단자전압 · 전류(역전류는 빨강) · 노심 | SCRAM / 재기동 |

**SCRAM은 0 V 전원이 아니라 노드 제거다**(`Ship.RebuildPowerNet`이 건너뛴다). 0 V로 두면 제일 약한 전원이 되어
역전류를 최대로 먹는다 - 끄는 것과 처지는 것은 다른 상태다. 그래서 죽어가는 원자로를 살리는 길이 둘이다:
SCRAM(그 전선은 남들에게 살아 있다)이나 차단기 개방(그 전선 전체가 죽는다).

입력은 `PauseControl.ClickToInspect`가 세계를 고르기 **전에** `ShipStatusHud.ClickSwitchboard`에 묻는다. 이 파일은
