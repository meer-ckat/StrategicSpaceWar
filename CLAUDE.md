...
- 그림도 자산이 아니다. 스프라이트 파일이 없고 `ArmorSkin`(판. 텍스처를 굽지 않는다 - 배 그림 + 6×6 손상 마스크를 `PlateSkin` 셰이더가 조합하고, CPU는 피해 시 마스크 36바이트만 쓴다)과 `SolidSkin`(모듈·탄, 단색)이 콜라이더에서 런타임에 만든다. **def의 색이 곧 아트다.** 서브셀 수식의 원본은 C# 한 벌이고 셰이더는 상수를 받아 조합만 한다.
