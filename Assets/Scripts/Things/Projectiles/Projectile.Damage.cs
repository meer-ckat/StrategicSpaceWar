...
public abstract partial class Projectile
{
    ...
    private void Explode(in HitResult r)
    {
        ...
        // **탄착점에 띄운다.** transform.position은 틱 행진이 끝나야 갱신되는 값이라
        // 여기(행진 도중)서는 이번 틱 시작 위치다 - 빠른 탄이면 폭발이 166m 뒤에서 핀다.
        Vector2 at = _surfaces.count > 0 ? _surfaces.hitPoint : (Vector2)transform.position;

        VfxOneShot.Play("Explosion", at, 4f, 0.7f, blastDamage * 0.0625f);
